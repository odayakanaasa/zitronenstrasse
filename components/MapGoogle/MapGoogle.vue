...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      const markerAdding = (placeIdArray) => {
        for (const [indexNumber, placeID] of placeIdArray.entries()) {
          new google.maps.places.PlacesService(this.map).getDetails({
            ...
            console.log(`++++ ARRAY GOOD : ${indexNumber + 1} ++++`)
      ...
      markerAdding(placeIdArray001)

      setTimeout(() => {
        markerAdding(placeIdArray002)
      }, 2000)
